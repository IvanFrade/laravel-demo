    ...
    <form action="{{ route('copies.update', $copy->id) }}" method="POST">
        ...
        <div class="mb-2">
            <label for="book_id">
                ...
                <select name="book_id" class="mt-0.5 w-full rounded border-gray-300 shadow-sm sm:text-sm p-2" required>
                    @foreach($books as $book)
                        <option value="{{ $book->id }}" @if($copy->book_id == $book->id) selected @endif>{{ $book->author }} - {{ $book->title }}</option>
                    @endforeach
                </select>
            </label>
        </div>
        <div class="mb-2">
            <label for="condition">
                ...
                <select name="condition" class="mt-0.5 w-full rounded border-gray-300 shadow-sm sm:text-sm p-2" required>
                    <option value="ottima" @if($copy->condition == 'ottima') selected @endif>Ottima</option>
                    <option value="buona" @if($copy->condition == 'buona') selected @endif>Buona</option>
                    <option value="accettabile" @if($copy->condition == 'accettabile') selected @endif>Accettabile</option>
                </select>
            </label>
        </div>
        <div class="mb-2">
            <label for="available">
                ...
                <select name="available" class="mt-0.5 w-full rounded border-gray-300 shadow-sm sm:text-sm p-2">
                    <option value="1" @if($copy->available) selected @endif>Disponibile</option>
                    <option value="0" @if(!$copy->available) selected @endif>Prenotata</option>
                </select>
            </label>
        </div>
        <div class="mb-2">
